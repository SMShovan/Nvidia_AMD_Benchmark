...
// ----------------------------------------------------------------------------
// Contended target: G warps share a scattered lock. `seed` (0 = deterministic default)
// is folded into the hash so --random gives a different but equally-uniform layout per run.
__device__ __forceinline__ long long node_lock_index(long long gwarp, long long numGroups,
  ...
  unsigned long long z = (unsigned long long)(groupId + (long long)j * numGroups)
...
// No-contention target: give base (a thread id, or a warp id) its OWN slot via a stride
// coprime with L=2^15*5^6 for the default 512M pool, so distinct bases never collide.
__device__ __forceinline__ long long private_index(long long base, long long L){
  return (long long)(((unsigned long long)base * 2654435761ULL) % (unsigned long long)L);
}

__global__ void k_spin_atomic_node(u64* counter, u64* sink,
                                   long long T, int W, long long L, int S, int G,
                                   int critsize, unsigned long long seed, int priv){
  long long tid = (long long)blockIdx.x * blockDim.x + threadIdx.x; if (tid >= T) return;
  (void)S;
  ...
  u64 mywork = 0;
  for (int j = 0; j < W; ++j){
    // private: each thread owns its counter (zero contention); else whole warp -> same lock.
    long long idx = priv ? private_index(tid, L) : node_lock_index(gwarp, numGroups, L, j, G, seed);
    atomicAdd(&counter[idx], 1ULL); mywork += crit_work(critsize, idx);
  }
  if (critsize > 0) atomicAdd(sink, mywork);
}
...
__global__ void k_spin_leader_node(int* lock, u64* counter, u64* abandoned, u64* sink,
                                   long long T, int W, long long L, int S, int G,
                                   int critsize, int maxspin, unsigned long long seed, int priv){
  long long tid = (long long)blockIdx.x * blockDim.x + threadIdx.x; if (tid >= T) return;
  (void)S;
  int lane = threadIdx.x % warpSize;
  long long ws = warpSize, gwarp = tid / ws;
  ...
  u64 mywork = 0;
  for (int j = 0; j < W; ++j){
    // private: each warp owns its lock (zero contention); else G warps share this lock.
    long long idx = priv ? private_index(gwarp, L) : node_lock_index(gwarp, numGroups, L, j, G, seed);
    if (lane == 0){
      int spins = 0; bool got = false;
      while (spins < maxspin){ if (atomicCAS(&lock[idx], 0, 1) == 0){ got = true; break; } ++spins; }
      if (got){ __threadfence(); counter[idx] += (u64)warpSize; mywork += crit_work(critsize, idx);
...
__global__ void k_spin_naive_node(int* lock, u64* counter, u64* abandoned, u64* sink,
  ...
  u64 mywork = 0;
  for (int j = 0; j < W; ++j){
    // private: each thread CAS-locks its OWN lock (no intra-warp competition -> AMD-safe).
    long long idx = priv ? private_index(tid, L) : node_lock_index(gwarp, numGroups, L, j, G, seed);
    int spins = 0; bool got = false;
    while (spins < maxspin){ if (atomicCAS(&lock[idx], 0, 1) == 0){ got = true; break; } ++spins; }
    if (got){ __threadfence(); counter[idx] += 1ULL; mywork += crit_work(critsize, idx);
...
inline void launch_spinlock_node(int variant, int* lock, u64* counter, u64* abandoned, u64* sink,
  ...
  dim3 block(tpb);
  dim3 grid((unsigned)((T + tpb - 1) / tpb));
  if      (variant == V_NAIVE)  k_spin_naive_node <<<grid, block>>>(lock, counter, abandoned, sink, T, W, L, S, G, critsize, maxspin, seed, priv);
  else if (variant == V_LEADER) k_spin_leader_node<<<grid, block>>>(lock, counter, abandoned, sink, T, W, L, S, G, critsize, maxspin, seed, priv);
  else                          k_spin_atomic_node<<<grid, block>>>(counter, sink, T, W, L, S, G, critsize, seed, priv);
  check_launch();
}
#else
...
inline long long private_index_cpu(long long base, long long L){
  return (long long)(((unsigned long long)base * 2654435761ULL) % (unsigned long long)L);
}
inline void launch_spinlock_node(int variant, int* lock, u64* counter, u64* abandoned, u64* sink,
                                 long long T, int W, long long L, int S, int G,
  ...
  if (variant == V_LEADER) {
    for (long long w = 0; w < numWarps; ++w)
      for (int j = 0; j < W; ++j) {
        long long idx = priv ? private_index_cpu(w, L) : node_lock_index_cpu(w, numGroups, L, j, G, seed);
        counter[idx] += (u64)ws;
      }
  } else {
    for (long long tid = 0; tid < T; ++tid){
      ...
      for (int j = 0; j < W; ++j) {
        long long idx = priv ? private_index_cpu(tid, L) : node_lock_index_cpu(gwarp, numGroups, L, j, G, seed);
        counter[idx] += 1ULL;
      }
    }
  }
